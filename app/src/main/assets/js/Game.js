function Game(boardElm, boardBackgroundElm){
    ...
    board.clicked = function(r, c){
        ...
        if(p instanceof HumanPlayer){
            console.log("r : " + r);
            console.log("c : " + c);
            console.log("isSet : " + board.isSet(r, c));
/*
        try{
            board.getPlaces()[suspendingR][suspendingC].elm.removeClass("suspending");
        }catch(e){
        }

        if( (suspendingC !== c || suspendingR !== r) && !board.isSet(r, c)){
            suspendingR = r;
            suspendingC = c;
            board.getPlaces()[r][c].elm.addClass("suspending");
            return ;
        }
*/
