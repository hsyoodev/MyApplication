...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public List<Map<String, Object>> select() {
        ...
        Cursor cursor =
                db.rawQuery("SELECT empno, ename, job, hiredate, deptno FROM emp", null);
        while (cursor.moveToNext()) {
            Map<String, Object> map = new HashMap<String, Object>();
            map.put("empno", cursor.getInt(0));
            map.put("ename", cursor.getString(1));
            map.put("job", cursor.getString(2));
            map.put("hiredate", cursor.getString(3));
            map.put("deptno", cursor.getInt(4));
            list.add(map);
        }
        db.close();
        return list;
